...
class posture_detector():
    ...
    def find_person(self, img, draw=True):
        ...
        # Comment out the default MediaPipe drawing to avoid overlap
    ...
    def find_landmarks(self, img, draw=True):
        ...
        if self.results.pose_landmarks:
            ...
            if draw:
                # Draw pretty pose after collecting all landmarks
                connections = list(self.mp_pose.POSE_CONNECTIONS)
                draw_pretty_pose(img, self.landmark_list, connections)
        return self.landmark_list

    def find_angle(self, img, p1, p2, p3, draw=True, landmark_list=None):
        """Calculate and optionally visualize the angle between three points."""
        # Use provided landmark_list or default to self.landmark_list
        lm_list = landmark_list if landmark_list is not None else self.landmark_list
        x1, y1 = lm_list[p1][1:]
...
def draw_pretty_pose(img, landmark_list, connections, color_line=(255, 0, 255), color_joint=(0, 255, 255)):
    """Draw attractive pose skeleton with thick lines, large joints, and landmark indices."""
    # Draw connections (lines)
    for connection in connections:
        start_idx, end_idx = connection
        if start_idx < len(landmark_list) and end_idx < len(landmark_list):
            x1, y1 = landmark_list[start_idx][1:]
            x2, y2 = landmark_list[end_idx][1:]
            cv2.line(img, (x1, y1), (x2, y2), color_line, 4)  # Thicker line

    # Draw joints (circles) and indices
    for idx, (_, x, y) in enumerate(landmark_list):
        cv2.circle(img, (x, y), 8, color_joint, cv2.FILLED)  # Larger joint
